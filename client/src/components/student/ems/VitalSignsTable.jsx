...
export default function VitalSignsTable({ title, table, value, onChange, points }) {
  ...
  return (
    <div className="ems-vitals-table-wrap">
      {title && <div className="ems-vitals-table-title">{title}</div>}
      {Number.isFinite(points) && <div className="ems-points-badge">{points} points</div>}
      <div className="ems-table-scroll">
        <table className="ems-table ems-table-entry">
          <thead>
            <tr>
          ...
          <tbody>
            {rows.map((row) => (
              <tr key={row.row_id}>
                <td className="ems-vitals-row-label">{row.label}</td>
