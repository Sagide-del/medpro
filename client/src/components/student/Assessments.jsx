  ...
  return (
    <div className="mcq-page question-bank-v2">
      ...
      <div className="question-bank-v2-content">
        <aside className="question-bank-v2-topics" aria-label="Question bank filters"><h2>Browse topics</h2>{[['all', 'All topics', enriched.length], ['critical', 'Critical', enriched.filter((module) => module.mastery < 60).length], ['mastered', 'Mastered', enriched.filter((module) => module.mastery > 80).length], ['progress', 'In progress', enriched.filter((module) => module.mastery >= 60 && module.mastery <= 79).length]].map(([value, label, count]) => <button type="button" key={value} className={filter === value ? 'is-active' : ''} onClick={() => setFilter(value)}>{label}<span>{count}</span></button>)}</aside>
        <main className="question-bank-v2-results"><div className="question-bank-v2-results-head"><h2>Topic paths</h2><span>{filtered.length} available</span></div><div className="question-bank-v2-list">{filtered.map((module) => { const band = masteryBand(module.mastery); return <article key={module.id} className={`question-bank-v2-module ${band.key}`}>
